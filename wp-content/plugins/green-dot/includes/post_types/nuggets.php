    ...
    echo '			<option value="gd_" ' . selected( $gd_nugget_post_type, 'gd_', false ) . '> ' . __( '', 'fanatic' ) . '</option>';
      ...
      foreach ($postTypes as $post) {
        if ( $post->name == $getPostType) {
          echo '<option value="' . $post->name . '" ' . selected . '> ' . __( $post->label, 'fanatic' ) . '</option>';
        }
        else if ( $post->name != 'attachment' ) {
          echo '<option value="' . $post->name . '" ' . selected( $gd_nugget_post_type, $post->name, false ) . '> ' . __( $post->label, 'fanatic' ) . '</option>';
        }
      }
      echo '<option value="' . 'page' . '" ' . selected( $gd_nugget_post_type, 'page' , false ) . '> ' . __( 'Page', 'fanatic' ) . '</option>';
		echo '			</select>';
